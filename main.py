...
axes[1].set_title('Градиент')
axes[2].set_title('Scale')
axes[3].set_title('DCT')
axes[4].set_title('Гистограмма')
axes[5].set_title('DFT')
# Обучение
for c in range(8, 9):
    ...
    for typeOfFaceNum in range(c, 15):
        ...
        for faceNum in range(1, 31):
            ...
            counter = Counter(resNumMany)
            popularOne = -1
            for i in counter:
                if counter[i] > popularOne:
                    popularOne = counter[i]
                    pict =  cv2.cvtColor(cv2.imread(str(i) + '-' + str(typeOfFaceNum) + '.jpg'), cv2.COLOR_BGR2GRAY)
                    resNum = i
            if obj[0] != False:
                obj[0].set_data(pict)
            else:
